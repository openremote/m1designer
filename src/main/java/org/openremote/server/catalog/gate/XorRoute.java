...
public class XorRoute extends GateRoute {

    private static final Logger LOG = LoggerFactory.getLogger(XorRoute.class);

    public XorRoute(CamelContext context, Flow flow, Node node) {
    ...
    protected void configureProcessing(ProcessorDefinition routeDefinition) throws Exception {
        ...
        routeDefinition
            .process(exchange -> {
                ...
                if (gate.a && !gate.b || !gate.a && gate.b) {
                    LOG.debug("a XOR b is true");
                    exchange.getIn().setBody(1);
                } else {
                    LOG.debug("a XOR b is false");
