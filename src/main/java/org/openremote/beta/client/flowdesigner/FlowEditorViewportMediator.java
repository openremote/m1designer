...
public class FlowEditorViewportMediator extends AbstractMediator {
    ...
    protected void onMouseWheel(NodeMouseWheelEvent event) {
        ...
        if (event.isSouth() != ZOOM_INVERT) {
            scaleDelta = 1 / (1 + ZOOM_FACTOR);
        } else {
            scaleDelta = 1 + ZOOM_FACTOR;
        }
    ...
    protected void onMultiTouchMove(List<TouchPoint> touches) {
        ...
        if (ZOOM_TOUCH_THRESHOLD > 0) {
            double delta = distance - lastTouchDistance;
            if ((delta > 0 && delta < ZOOM_TOUCH_THRESHOLD)
                || (delta <0 && delta > -ZOOM_TOUCH_THRESHOLD))
                return;
        }
        ...
        if (zoomOut != ZOOM_INVERT) {
            scaleDelta = 1 / (1 + ZOOM_FACTOR * 2);
        } else {
            scaleDelta = 1 + ZOOM_FACTOR * 2;
        }
